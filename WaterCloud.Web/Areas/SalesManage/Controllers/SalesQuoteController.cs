...
namespace WaterCloud.Web.Areas.SalesManage.Controllers
{
    ...
    {
        public SalesQuoteService _service { get; set; }
        #region 获取数据
        [HandlerAjaxOnly]
        [IgnoreAntiforgeryToken]
        public async Task<ActionResult> GetGridJson(SoulPage<SalesQuoteEntity> pagination, string keyword)
        {
        ...
        [HttpPost]
        [HandlerAjaxOnly]
        public async Task<ActionResult> SubmitForm(SalesQuoteEntity entity, string keyValue)
        {
            try
            {
        ...
        public async Task<ActionResult> ApproveForm(string keyValue)
        {
            try
            {
                ...
                return await Success("审核通过，已生成销售订单。", "", keyValue, DbLogType.Submit);
            }
            catch (Exception ex)
            {
