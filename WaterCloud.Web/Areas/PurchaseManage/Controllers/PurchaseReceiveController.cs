...
namespace WaterCloud.Web.Areas.PurchaseManage.Controllers
{
    ...
    {
        public PurchaseReceiveService _service { get; set; }
        #region 获取数据
        [HandlerAjaxOnly]
        [IgnoreAntiforgeryToken]
        public async Task<ActionResult> GetGridJson(SoulPage<PurchaseReceiveEntity> pagination, string keyword)
        {
        ...
        [HttpPost]
        [HandlerAjaxOnly]
        public async Task<ActionResult> SubmitForm(PurchaseReceiveEntity entity, string keyValue)
        {
            try
            {
        ...
        public async Task<ActionResult> ReturnForm(string keyValue)
        {
            try
            {
                ...
                return await Success("已生成采购退货单。", "", keyValue, DbLogType.Submit);
            }
            catch (Exception ex)
            {
