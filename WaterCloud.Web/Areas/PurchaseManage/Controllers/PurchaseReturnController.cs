...
namespace WaterCloud.Web.Areas.PurchaseManage.Controllers
{
    ...
    {
        public PurchaseReturnService _service { get; set; }
        #region 获取数据
        [HandlerAjaxOnly]
        [IgnoreAntiforgeryToken]
        public async Task<ActionResult> GetGridJson(SoulPage<PurchaseReturnEntity> pagination, string keyword)
        {
        ...
        [HttpPost]
        [HandlerAjaxOnly]
        public async Task<ActionResult> SubmitForm(PurchaseReturnEntity entity, string keyValue)
        {
            try
            {
